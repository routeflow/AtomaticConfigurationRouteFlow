...
class RFServer(RFProtocolFactory, IPC.IPCMessageProcessor):
    def threaded_function_on_switch_join(self, entries):
...
##write config file of lxc
	   ...
	   f.write(Net)
	   stri=entries[1]
	   hw="lxc.network.hwaddr = "+str(stri[0])+str(stri[1])+":" +str(stri[3])+str(stri[4])+":"+str(stri[6])+str(stri[7])+":"+str(stri[9])+str(stri[10])+":"+str(stri[12])+str(stri[13])+":"+str(stri[15])+str(stri[16])
	   f.write(hw)
...
## make ospf.conf
	   ...
	   while y<length:
		...
		if entries[y]=="NEW":
		   y=y+1
		   datapathid[entries[y]]=entries[y+1]
		   print datapathid[entries[y]]
		   print entries[y]
		y=y+1
##OSPF conf	
...
##ZEBRA conf	
	   ...
	   for x in datapathid:
		L="interface eth"+str(x)
		f.write(L)
		L="\n        ip address "+ datapathid[str(x)] + "/24" + "\n!\n"
	        f.write(L)
	   f.close()
#copy passwd files
	   command="cp conf/passwd /var/lib/lxc/"+lxcname+"/rootfs/etc/"
	   result_code = os.system(command)
	   command="cp conf/shadow /var/lib/lxc/"+lxcname+"/rootfs/etc/"
	   result_code = os.system(command)
#starting lxc
	   #command="cp rftest/quagga"+str(int(entries[1]))+"/*  "+ "/var/lib/lxc/"+lxcname+"/rootfs/etc/quagga"
	   #print command;
	   #result_code = os.system(command)
           #self.config = RFConfig(filename,1)
	   #command="lxc-start -n "+lxcname+ " -d"
	   command='./rftest/lxcstart ' + lxcname
	   result_code = os.system(command)
...
#starting control plane
	   ...
	   for x in range(1,a):
		...
		name=lxcname+ "." +str(x)
		command='./rftest/rfserverstart '+ lxcname+ "." +str(x)
		print command
	        result_code = os.system(command)
		command="\n"+dp+","+str(x)+",0,"+dp+","+str(x)
		f.write(command)
	...
	if entries[0]=="\"join":
	   self.switch[entries[1]]=0
           t = threading.Thread(threaded_function_on_switch_join,args=(entries,))
	   t.start()
	   return
...
if __name__ == "__main__":
    ...
    try:
	print "registering for functions"
