...
def on_datapath_up(event):
    ...
    for port_num, port_addr in ports:
        if port_num <= OFPP_MAX:
            msg = DatapathPortRegister(ct_id=ID, dp_id=dp_id, dp_port=port_num)
            ipc.send(RFSERVER_RFPROXY_CHANNEL, RFSERVER_ID, msg)
            log.info("Registering datapath port (dp_id=%s, dp_port=%d)",
                     format_id(dp_id), port_num)
    ...
    def process(self, from_, to, channel, msg):
        ...
        if type_ == ROUTE_MOD:
            ...
            if send_of_msg(msg.get_id(), ofmsg) == SUCCESS:
                log.info("routemod sent to datapath (dp_id=%s) %d",
                         format_id(msg.get_id()), len(msg))
            else:
